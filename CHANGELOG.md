...
- Batch decoded result inserts and covering-monitor cursor updates per atomic block commit, rejecting duplicate result identities transactionally.
...
- Return concrete block numbers from cursor and monitor next-block calculations instead of optional values.
- Rename autogenerated unnamed ABI parameters from `arg_<index>` to `arg<index>` for newly created monitors, affecting schemas, filter fields, result columns, and result JSON keys.
